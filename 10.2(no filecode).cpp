...
public:
    ...
    ~HuffmanTree();
    [[nodiscard]] int getHuffmanCodeLength(); //获取长度
    void code();
private:
    int *weight = new int[26];
    string file = "";
    int *huffmanCodeSingle = new int[26];
    HuffmanNode *root = NULL;
    void clean(const HuffmanNode *root);
    void MakeHuffmanTree(); //建树
    void find(); //找出单个字符编码长度
    void DFSFind(const HuffmanNode *theRoot , const int& front); //找字符编码长度的递归函数
...
void HuffmanTree::DFSFind(const HuffmanNode* theRoot , const int& front)
{
    if(theRoot == NULL)
        return;
...
int HuffmanTree::getHuffmanCodeLength()
{
    this->find();
    int result = 0;
    for(int count = 0 ; count < 26 ; ++count)
    {
        if(weight[count] != 0)
            result += huffmanCodeSingle[count] * weight[count];
    }
    return result;
}
